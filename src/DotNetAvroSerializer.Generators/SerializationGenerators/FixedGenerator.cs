...
internal static class FixedGenerator
{
    internal static void GenerateSerializationSourceForFixed(AvroGenerationContext context)
    {
        ...
        context.AsyncSerializationCode.AppendLine(@$"if ({context.SourceAccessor}.Length != {schema!.Size}) throw new AvroSerializationException(""Byte array {context.SourceAccessor} has to be of a fixed length of {schema.Size} but found {{{context.SourceAccessor}.Length}}"");");
        context.SerializationCode.AppendLine($@"BytesSchema.Write(outputStream, {context.SourceAccessor});");
        context.AsyncSerializationCode.AppendLine($@"await BytesSchema.WriteAsync(outputStream, {context.SourceAccessor}, cancellationToken);");
    }
}
