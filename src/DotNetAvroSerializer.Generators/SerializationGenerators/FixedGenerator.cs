...
internal static class FixedGenerator
{
    internal static void GenerateSerializationSourceForFixed(AvroGenerationContext context)
    {
        ...
        context.SerializationCode.WriteLine(@$"if ({context.SourceAccessor}.Length != {schema!.Size}) throw new AvroSerializationException(""Byte array {context.SourceAccessor} has to be of a fixed length of {schema.Size} but found {{{context.SourceAccessor}.Length}}"");");
        context.SerializationCode.WriteLine(context.WriteCall("BytesSchema", context.SourceAccessor));
    }
}
